...
const initialState = {
  ...
  postHistories: {}, // Object to store post history: { postId: [history array] }
...
const postSlice = createSlice({
  ...
  reducers: {
      ...
      if (postIndex !== -1) {
        ...
        if (commentIndex !== -1) {
          const existingReaction =
            state.posts[postIndex].comments[commentIndex].reactions.find(
              (reaction) => reaction.userId === action.payload.userId
            );
          if (existingReaction) {
	...
	recordPostHistory: (state, action) => {
		const { postId, content } = action.payload;
		if (!state.postHistories[postId]) {
		  state.postHistories[postId] = [];
		}
		state.postHistories[postId].push(content);
	},
	recordCommentHistory: (state, action) => {
		const { postId, commentId, content } = action.payload;
		if (!state.commentHistories[postId]) {
		  state.commentHistories[postId] = {};
		}
		if (!state.commentHistories[postId][commentId]) {
		  state.commentHistories[postId][commentId] = [];
		}
		state.commentHistories[postId][commentId].push(content);
	},
	...
	editComment: (state, action) => {
		...
		if (postIndex !== -1) {
		  ...
		  if (commentIndex !== -1) {
			state.posts[postIndex].comments[commentIndex].text = text;
		  }
		}
	  },
  },
