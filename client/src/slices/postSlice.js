...
const postSlice = createSlice({
  ...
  reducers: {
    setPosts: (state, action) => {   // Reducer to set multiple posts
      state.posts = action.payload.posts;  // Set posts from action payload
    },
    addPost: (state, action) => {   // Reducer to add a new post
      state.posts.push(action.payload.post);  // Add new post to the posts array
    },
    updatePost: (state, action) => {   // Reducer to update a post
      const index = state.posts.findIndex(post => post._id === action.payload.post._id);  // Find the index of the post to update
      if (index !== -1) {
        state.posts[index] = action.payload.post;  // Update the post in the array
      }
    },
    deletePost: (state, action) => {   // Reducer to delete a post
      state.posts = state.posts.filter(post => post._id !== action.payload.postId);  // Remove post by ID
    },
  },
});
