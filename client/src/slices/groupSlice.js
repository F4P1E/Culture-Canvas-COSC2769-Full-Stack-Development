...
const groupSlice = createSlice({
  ...
  reducers: {
    ...
    fetchGroupsInfo: (state, action) => {
      state.groupInfo = action.payload;  // Store the detailed information of all groups
    },
    setCurrentGroup: (state, action) => {
      state.currentGroupId = action.payload.groupId; // Update the current group ID

      console.log(`Payload: ${action.payload}`);
      console.log(`ID from Payload: ${action.payload.groupId}`);
      console.log(`State: ${state}`);
      console.log(`ID from State: ${state.currentGroupId}`);
    },
    setRequests: (state, action) => {   // Reducer to set the list of join requests
    ...
    setMembers: (state, action) => {
      ...
      // Find the group that should have its members updated
    ...
    requestJoinGroup: (state, action) => {
      ...
      if (group) {
        state.requests.push({ groupId: action.payload.groupId, userId: action.payload.userId });
      }
    },
    ...
    approveJoinRequest: (state, action) => {
      ...
      if (group) {
        ...
        state.requests = state.requests.filter(request => request.userId !== action.payload.userId);
      }
    },
    ...
    deleteMemberFromGroup: (state, action) => {
      ...
      if (group) {
        ...
        group.members = group.members.filter(memberId => memberId !== action.payload.userId);
      }
    },
