...
import { fetchGroupsInfo } from '../../slices/groupSlice'; 
...
const JoinedGroupList = () => {
  ...
  const handleViewGroup = async (groupId) => {
    ...
    const data = await response.json(); // Parse the response data
      dispatch(fetchGroupsInfo(data));  // Update Redux store with fetched groups
    ...
    <div>
      <ul>
        {groups.map((group) => (
          <li key={group._id}>
            {group.name}
            <button onClick={() => handleViewGroup(group._id)}>View Group</button>
          </li>
        ))}
      </ul>
    </div>
